...
class AgentState:
    ...
    def copy( self ):
        ...
        state.angryTimer  = self.angryTimer
        return state
...
class NetworkGame(Game):
    def __init__(self, srv, ip, agents, display, rules, startingIndex=0, muteAgents=False, catchExceptions=False ):
        super(NetworkGame, self).__init__(agents, display, rules, startingIndex, muteAgents, catchExceptions)
        self.server = srv
        self.host = ip
        self.port = 50000
        self.catchExceptions = False
        # i.e.: Server makes first step
        self.startingIndex = 0 if self.server else 1
    ...
    def runAsServer(self):
        print('Running as server and waiting for connections ... ')
        sr = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        sr.bind(('0.0.0.0', self.port))
        sr.listen(1)
        self.socket, addr = sr.accept()
        print('Client connected')
    
    def runAsClient(self):
        print('Running as client and trying to connect to server at ' + self.host + '  ... ')
        try:
