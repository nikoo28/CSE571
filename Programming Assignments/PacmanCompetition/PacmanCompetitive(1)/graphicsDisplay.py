...
PACMAN_MAD_COLOR = formatColor(204.0/255.0,0.0/255.0,0.0/255)
PACMAN_SCALE = 0.5
#pacman_speed = 0.25
...
class PacmanGraphics:
    ...
    def update(self, newState):
        ...
        if agentState.isPacman:
            ...
            self.animateRemoteAgent(agentState, prevState, prevImage)
        ...
        if newState._capsuleEaten != None:
            for c in self.capsules:
                self.removeCapsule(c, self.capsules)
            self.capsules = self.drawCapsules(newState.layout.capsules)
            newState._capsuleEaten = None
            newState._capsuleSpawned = None
    ...
    def make_window(self, width, height):
        ...
        begin_graphics(screen_width,
                       screen_height,
                       BACKGROUND_COLOR,
                       "CSE 571 Competitive Pacman")

    def drawPacman(self, pacman, index, mad=False):
        ...
        if mad:
            fillColor = PACMAN_MAD_COLOR
            outlineColor = PACMAN_MAD_COLOR
    ...
    def animateRemoteAgent(self, agent, prevAgent, image):
        if agent.angryTimer > 0:
            edit(image[0], ('fill', PACMAN_MAD_COLOR), ('outline', PACMAN_MAD_COLOR))
        else:
            edit(image[0], ('fill', PACMAN_COLOR), ('outline', PACMAN_COLOR))
        self.moveRemoteAgent(self.getPosition(agent), self.getDirection(agent), image)
        refresh()
    
    def moveRemoteAgent(self, position, direction, image):
